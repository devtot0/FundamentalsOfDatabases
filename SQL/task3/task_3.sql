...
--Assuming that the number of people per department was meant:
SELECT job_id, count(job_id)
FROM employees
GROUP BY job_id;
 
--Assuming that the count of people who have a job, that is done by more than one person was meant:
SELECT SUM(myCol)
FROM (
...
SELECT department_name, street_address, city, postal_code, state_province, country_name
